...
export class ReportingService {
    ...
    async getGlobalMISSummary() {
        const [pendingCount, escalatedCount, totalDecisions] = await Promise.all([
            this.prisma.decision.count({ where: { status: 'PENDING_APPROVAL' } }),
            this.prisma.decision.count({ where: { status: 'ESCALATED' } }),
        ...
        return {
            ...
            complianceScore: totalDecisions > 0 ? 94 : 0, // Placeholder for complex compliance logic
            totalDecisions
        };
    }
